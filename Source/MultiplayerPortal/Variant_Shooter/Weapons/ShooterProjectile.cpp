...
{
	// ignore if we've already hit something else or hit a portal
	
	if (bHit || OtherComp->ComponentHasTag("PortalTrigger"))
	{
		return;
	}
...
void AShooterProjectile::ProcessHit(AActor* HitActor, UPrimitiveComponent* HitComp, const FVector& HitLocation, const FVector& HitDirection)
{
	...
	if (ACharacter* HitCharacter = Cast<ACharacter>(HitActor))
	{
		...
		if (HitCharacter != GetOwner() || bDamageOwner)
		{
			...
			FDamageEvent DamageEvent{HitDamageType};
			HitCharacter->TakeDamage(HitDamage, DamageEvent, GetInstigator()->GetController(), this);
		}
	}
...
bool AShooterProjectile::GetCanTeleport()
{
	return CanTeleport;
}

void AShooterProjectile::HasBeenTeleported()
{
	CanTeleport = false;
	CollisionComponent->SetCollisionEnabled(ECollisionEnabled::NoCollision);
	GetWorld()->GetTimerManager().SetTimer(TeleportHandle, this, &AShooterProjectile::ResetCanTeleport, 0.25f);
}

void AShooterProjectile::ResetCanTeleport()
{
	CanTeleport = true;
	CollisionComponent->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
}
